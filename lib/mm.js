...
var MM = {
    ...
    /**
     * Save the geolocation and device info 
     * in the database - table log.
     * @return {void}
     */
    saveGeoInfo: function(gtoken,guserid) {
		$.ajax({
			type: 'POST',
			url: MM.siteurl+'/webservice/rest/server.php?moodlewsrestformat=json',
			data: "wsfunction=local_start_geoinfo&wstoken="+gtoken+"&dados[userid]="+guserid+"&dados[geoinfo]="+$("#deviceinfo").val(),
			dataType: 'json',
    ...
    setUpConfig: function() {
		...
		//MM.setConfig('current_token', "ede23c17d214d17c11959d303a344490");
            ...
            success:function() {
                ...
                MM._checkMobileLocalPlugin(siteurl,
                    ...
                    function() {
						...
						// @start - authentication user login 
                        // MM._expandAddSiteForm();
						MM.addSite();
                    }
                );
    ...
    _saveToken: function(token) {
        ...
        MM.moodleWSCall('moodle_webservice_get_siteinfo', {}, function(site) {
			...
			MM.saveGeoInfo(MM.config.start_token,site.userid);		
        }, preSets);
    ...
    saveSite: function(username, password, siteurl) {
        ...
        // Now, we try to get a @start token.
        $.ajax({
            url:loginURL,
            type:'POST',
            data:{
            ...
            success:function(json) {
                if (typeof(json.token) != 'undefined') {
					MM.setConfig('start_token', json.token);				
                } else {
                    var error = MM.lang.s('invalidaccount');
    ...
    /**
     * A wrapper function for a moodle WebService External Function call @start_token.
     *
	 * @param {string} start_token The WebService token to be called.
     * @param {string} method The WebService method to be called.
     * @param {Object} data Arguments to pass to the method.
     * @param {Object} callBack Function to be called in success.
     * @param {Object} preSets Extra settings
     ...
     *      silently For not raising erronors.
     */	 
    moodleWSextCall: function(method, data, callBack, preSets, errorCallBack) {
        ...
        data.wsfunction = method;
        //data.wstoken = preSets.wstoken;
		data.wstoken = MM.config.start_token;
     ...
     */
    refresh: function(malert) {
		if (typeof malert === 'undefined') { malert = 0; }
        ...
        if (MM.deviceConnected()) {
            ...
            MM.moodleWSCall('moodle_webservice_get_siteinfo', {}, function(site) {
                ...
                MM.cache.invalidate();
                if (malert == 0) MM.popMessage(MM.lang.s('allcachesinvalidated'));
           ...
           if (malert == 0) MM.popMessage(MM.lang.s('norefreshdisconnected'));
        }
    },
    ...
    logoutUser: function() {
		/**
		* @start - destroy cache authentication
		*
		*/
		var siteId = MM.getConfig('current_site');
		siteId = siteId.id
